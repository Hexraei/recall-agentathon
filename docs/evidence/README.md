...
Three defects, three different causes, one shared method: **in each case the first
...
The other pattern worth naming: **two of the three fixes made the product better on the
merits, not just more robust.** Bounding the report schema (01) produced a page a student
...
- **The checker enforces its rules on structured verdicts but not on free-text prose.** A
  ...
  overstate.
- **Bug 04 is confirmed, not fixed.** We found it reviewing real students' reports and
  ...
  re-measured against the same real batch, the same way bugs 01 and 03 were, before it
  gets called done.
